...
class ADDONNAME_OT_fcoperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        settings.landmarks_model_path = bpy.path.abspath(settings.landmarks_model_path)
        settings.landmarks_json_export = bpy.path.abspath(settings.landmarks_json_export)
        ...
        if settings.capture_mode == 'camera':
            ...
            if settings.want_to_record and len(settings.output_video) == 0:
                has_error = True
                self.report({'ERROR'}, 'Please insert a output video path. Ex: /home/workspace/videoname.avi')
        else:
            ...
            if len(settings.input_video) == 0:
                has_error = True
                self.report({'ERROR'}, 'Please insert a input video path. Ex: /home/workspace/videoname.avi')
